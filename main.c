...
static const char key_map[256] = {
    [VK_SHIFT] = MU_KEY_SHIFT,   [VK_CONTROL] = MU_KEY_CTRL,   [VK_MENU] = MU_KEY_ALT,
    [VK_RETURN] = MU_KEY_RETURN, [VK_BACK] = MU_KEY_BACKSPACE,
};
...
LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_LBUTTONUP:
    ...
    return 0;
  case WM_MBUTTONDOWN:
    mu_input_mousedown(ctx, mouse_x, mouse_y, MU_MOUSE_MIDDLE);
    return 0;
  case WM_MBUTTONUP:
    mu_input_mouseup(ctx, mouse_x, mouse_y, MU_MOUSE_MIDDLE);
    return 0;
  case WM_RBUTTONDOWN:
    mu_input_mousedown(ctx, mouse_x, mouse_y, MU_MOUSE_RIGHT);
    return 0;
  case WM_RBUTTONUP:
    mu_input_mouseup(ctx, mouse_x, mouse_y, MU_MOUSE_RIGHT);
    return 0;
  case WM_MOUSEMOVE: {
  ...
  case WM_MOUSEWHEEL: {
    ...
    mu_input_scroll(ctx, 0, -dy * 0.5f);
    return 0;
  }
  case WM_KEYDOWN:
    mu_input_keydown(ctx, key_map[wparam]);
    return 0;
  case WM_KEYUP:
    mu_input_keyup(ctx, key_map[wparam]);
    return 0;
  case WM_CHAR: {
    char str[] = {(char)wparam, 0};
    if (str[0] != '\b') {
